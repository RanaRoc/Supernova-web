...
this.showPose = true;
  ...
  getResults(): void {
    // Votre logique actuelle pour obtenir les résultats
this.showFinition =false;
this.showResponse = true;
   /*forkJoin({
      responses: this.responseService.getResponsesByStyle(this.selectedStyle, this.selectedEspace, this.selectedFinition, this.selectedForme, this.selectedMaterial).valueChanges(),
      products: this.productService.getAll().valueChanges()
    })
    .pipe(
      takeUntil(this.unsubscribe$),
      map(({ responses, products }) => {
        // Filtrez les réponses pertinentes
        ...
        // Obtenez les CAS des réponses filtrées
        const selectedCas = filteredResponses.map(response => response.Cas);

        // Filtrez les produits par les CAS sélectionnés
        const selectedProducts = products.filter(product => selectedCas.includes(product.Cas));

        return selectedProducts;
      })
    )
    .subscribe((selectedProducts: Product[]) => {
      // Utilisez les produits filtrés
